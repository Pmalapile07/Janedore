...
function getAllProductImages(product, variantIndex) {
  ...
  // For non-jewelry: ghost images only in main slider, model+detail go to secondary
  const ghost = (variant.images?.ghost || []).filter(Boolean);
  if (ghost.length) return ghost;
  // fallback to model if no ghost
  const model = (variant.images?.model || []).filter(Boolean);
...
function handleStickyAddClick(productId) {
  ...
  if(!S.stickyExtended) {
    // First click: extend the bar (hide button, show sizes/swatches, shift upward)
    bar.classList.add('extended');
    S.stickyExtended = true;

    // Calculate the height and shift upward so it grows from the top
    setTimeout(() => {
...
function addToCartFromDetailSticky(id) {
  ...
  openCart();
  // Reset the sticky bar after adding to cart
  const bar=document.getElementById('sticky-add-bar');
  if(bar){
    ...
    S.stickyExtended=false;
    S.selectedSize = null;
    // Re-render sizes without selection
