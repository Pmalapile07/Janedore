...
function loadWishlistFromStorage() { try { const saved = localStorage.getItem('janedore_wishlist'); if (saved) { const ids = JSON.parse(saved); S.wishlist = ids.map(id => PRODUCTS.find(p => p.id === id)).filter(Boolean); } } catch(e) { S.wishlist = []; } }
function saveWishlistToStorage() { try { const ids = S.wishlist.map(p => p.id); localStorage.setItem('janedore_wishlist', JSON.stringify(ids)); } catch(e) {} }
...
function getProductImages(product, variantIndex) { const idx = variantIndex !== undefined ? variantIndex : (S.productVariantSelections[product.id] ?? 0); const variant = product?.variants?.[idx] ?? product?.variants?.[0] ?? {}; if (product.category === 'jewelry') return [...(variant.images?.model||[]), ...(variant.images?.ghost||[])].filter(Boolean).length ? [...(variant.images?.model||[]), ...(variant.images?.ghost||[])].filter(Boolean) : [PLACEHOLDER_IMAGE]; return variant.images?.[S.imageMode] || variant.images?.ghost || variant.images?.model || [PLACEHOLDER_IMAGE]; }
function getProductThumbnail(product, variantIndex) { return safeImage(getProductImages(product, variantIndex)[0]); }
function getAllProductImages(product, variantIndex) { const idx = variantIndex !== undefined ? variantIndex : (S.productVariantSelections[product.id] ?? 0); const variant = product?.variants?.[idx] ?? product?.variants?.[0] ?? {}; const detail = (variant.images?.detail || []).filter(Boolean); if (product.category === 'jewelry') return [...(variant.images?.model||[]), ...(variant.images?.ghost||[]), ...detail].map(safeImage).filter(Boolean).length ? [...(variant.images?.model||[]), ...(variant.images?.ghost||[]), ...detail].map(safeImage) : [PLACEHOLDER_IMAGE]; return [...(variant.images?.ghost||[]), ...(variant.images?.model||[]), ...detail].map(safeImage) || [PLACEHOLDER_IMAGE]; }
function getSecondaryImages(product, variantIndex) { const idx = variantIndex !== undefined ? variantIndex : (S.productVariantSelections[product.id] ?? 0); const variant = product?.variants?.[idx] ?? product?.variants?.[0] ?? {}; const detail = (variant.images?.detail || []).filter(Boolean); const model = (variant.images?.model || []).filter(Boolean); return [...model, ...detail]; }
...
function createStickyBar(product) { removeStickyBar(); const soldOut=isProductSoldOut(product); const vi=S.productVariantSelections[product.id]??0; const price=product.salePrice?product.salePrice:product.price; const bar=document.createElement('div'); bar.id='sticky-add-bar'; bar.className='sticky-add-bar'; bar.innerHTML=`<div class="sticky-name">${product.name||''}</div><div class="sticky-price">${formatPrice(price)}</div><div class="sticky-extras"><div class="sticky-sizes" id="sticky-sizes">${(product.sizes||[]).map(s=>`<button class="sticky-size-btn" onclick="event.stopPropagation();selectStickySize(this,'${s}')">${s}</button>`).join("")}</div><div class="sticky-swatches" id="sticky-swatches">${stickySwatchesHtml(product,vi)}</div></div><button class="sticky-add-btn" onclick="handleStickyAddClick('${product.id}')"${soldOut?' disabled':''}>${soldOut?'Sold Out':'Add to Bag'}</button>`; document.body.appendChild(bar); updateStickyBarOnScroll(); }
...
function addToCart(productId,size) { const product=PRODUCTS.find(p=>p.id===productId); if(!product||isProductSoldOut(product))return; const vi=S.productVariantSelections[productId]??0; const variant=(product.variants||[])[vi]??{}; const existing=S.cart.find(i=>i.productId===productId&&i.size===(size||product.sizes[0])&&i.variantIndex===vi); if(existing)existing.qty++; else S.cart.push({productId,variantIndex:vi,size:size||product.sizes[0]||'OS',qty:1,name:product.name,brand:product.brand,price:product.price,salePrice:product.salePrice,color:variant.color||'Default',thumbnail:getProductThumbnail(product,vi)}); updateBadges();renderCart();saveCartToStorage(); }
...
function toggleWish(productId) { const product=PRODUCTS.find(p=>p.id===productId); if(!product)return; const idx=S.wishlist.findIndex(w=>w.id===productId); if(idx>=0)S.wishlist.splice(idx,1); else S.wishlist.push(product); updateBadges();renderWishlistPage();saveWishlistToStorage(); }
function renderWishlistPage() { if(!DOM.wishPageContent)return; if(!S.wishlist.length){DOM.wishPageContent.innerHTML='<div class="wish-page-empty"><div class="wish-page-empty-title">Your wishlist is empty</div><button class="btn-continue-shopping" onclick="navigateTo(\'products\')">Continue Shopping</button></div>';return;} DOM.wishPageContent.innerHTML=`<div class="wish-page-title">Wishlist (${S.wishlist.length})</div><div class="wish-page-grid">${S.wishlist.map(p=>{const vi=S.productVariantSelections[p.id]??0;const priceHtml=p.salePrice?`<span class="product-price-sale">${formatPrice(p.salePrice)}</span><span class="product-price-original">${formatPrice(p.price)}</span>`:formatPrice(p.price);const badgeLabel=p.badge==="sold"?"Sold Out":p.badge==="new"?"New":p.salePrice?"Sale":"";const badgeHtml=badgeLabel?`<div class="product-badge-wrap"><span class="badge-${p.badge==='sold'?'sold':p.salePrice?'sale':'new'}">${badgeLabel}</span></div>`:"";const allImages=getAllProductImages(p,vi);const slidesHtml=allImages.map(u=>`<div class="product-card-slide" style="background-image:url('${u}');"></div>`).join("");const barsHtml=allImages.length>1?`<div class="card-slider-bars">${allImages.map((_,i)=>`<div class="card-slider-bar${i===0?' active':''}"></div>`).join("")}</div>`:'';return `<div class="product-card${isProductSoldOut(p)?' sold-out':''}" data-product-id="${p.id}" onclick="goToProduct('${p.id}')"><div class="product-img-wrap" ontouchstart="cardTouchStart(event,'${p.id}')" ontouchend="cardTouchEnd(event,'${p.id}')"><div class="product-card-slides" id="card-slides-wish-${p.id}">${slidesHtml}</div>${barsHtml}${badgeHtml}</div><div class="product-brand-tag">${p.brand||''}</div><div class="product-name collection-name">${truncateName(p.name)}</div><div class="product-price-row"><div class="product-price">${priceHtml}</div></div></div>`;}).join("")}</div>`; }
