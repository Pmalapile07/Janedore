...
function buildNewsletterSection() {
  ...
  DOM.homepageNewsletterSection.innerHTML = `<div class="newsletter-section"><div class="newsletter-title">Subscribe to our newsletter</div><div class="newsletter-form"><input class="newsletter-input" type="email" placeholder="Enter your email" id="newsletter-email"><button class="newsletter-btn" onclick="subscribeNewsletter(document.getElementById('newsletter-email').value)"><svg viewBox="0 0 24 24"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></button></div></div>`;
}
...
function removeFromCart(productId, size, vi) { S.cart=S.cart.filter(i=>!(i.productId===productId&&i.size===size&&i.variantIndex===vi)); updateBadges(); renderCart(); }
function changeQty(productId, size, delta, vi) { const item=S.cart.find(i=>i.productId===productId&&i.size===size&&i.variantIndex===vi); if(item) item.qty=Math.max(1,item.qty+delta); renderCart(); }
...
function toggleWish(productId) {
  ...
  updateBadges(); renderWishlistPage();
...
function buildFooter(id) {
  ...
  const collapseHTML = sections.map(sec => {
    const links = { shop:["New In","Dresses","Tops","Bottoms","Jackets","Sets","Bags","Jewelry","Parfum","Sale"], brands:["JANEDORE","NIRIUS CO","THATO"], policies:["About","Shipping Policy","Return Policy","Privacy Policy","Terms & Conditions"], help:["FAQ","Size Guide","Shipping","Returns","Contact"] }[sec];
  ...
  el.innerHTML = `<div class="footer-top">${collapseHTML}</div><p class="footer-about">Janedore is a curated multi-brand fashion destination rooted in South Africa.</p><div class="footer-currency-lang"><div class="footer-currency" onclick="toggleFooterDropdown('currency-${id}','${id}')"><span class="footer-currency-label">${currLabel}</span><svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg><div class="currency-dropdown" id="currency-dropdown-${id}"><div class="dropdown-option" onclick="event.stopPropagation();selectCurrency('ZAR')">ZAR R</div><div class="dropdown-option" onclick="event.stopPropagation();selectCurrency('BWP')">BWP P</div><div class="dropdown-option" onclick="event.stopPropagation();selectCurrency('USD')">USD $</div><div class="dropdown-option" onclick="event.stopPropagation();selectCurrency('LSL')">LSL M</div><div class="dropdown-option" onclick="event.stopPropagation();selectCurrency('NAD')">NAD N$</div></div></div><div class="footer-lang" onclick="toggleFooterDropdown('lang-${id}','${id}')">EN <svg viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg><div class="lang-dropdown" id="lang-dropdown-${id}"><div class="dropdown-option">EN</div></div></div></div><div class="footer-bottom"><div class="footer-copy">© 2025 JANEDORE. ALL RIGHTS RESERVED.</div></div>`;
}
...
function renderSearchDefault() { DOM.searchBody.innerHTML='<div class="search-suggestions"><div class="search-suggestions-title">Popular Searches</div><div class="search-suggestion-pills">'+["Sunglasses","Jewelry","Pouch","Earrings","Parfum"].map(s=>`<button class="search-pill" onclick="searchFor('${s}')">${s}</button>`).join("")+'</div></div>'; }
...
async function init() {
  ...
  ["main-footer","products-footer","category-footer","campaign-footer","cart-footer","wishlist-footer","editorial-footer","checkout-footer"].forEach(buildFooter);
  buildCampaignSlider();
  const route = getRouteFromHash();
  if (route.page === 'product-detail') goToProduct(route.productId);
  else if (route.page === 'category') navigateToCategory(route.cat);
