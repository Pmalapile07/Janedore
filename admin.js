(function () {
  ...
  function loadUserRole(user) {
    return adminsRef.doc(user.uid).get().then(function(doc) {
      if (doc.exists) {
        ...
        window._currentVendorId = (window._currentUserRole === 'VENDOR') ? (data.vendorId || null) : null;
      } else {
  ...
  function renderRoleUI() {
    ...
    if (badge) {
      ...
      badge.textContent   = isSuperAdmin() ? 'Super Admin' : (window._currentUserRole === 'VIEWER' ? 'Viewer' : 'Vendor');
  ...
  window.switchTab = function(tab) {
    ...
    if (tab !== 'messages') {
      ...
      window._detachActiveChatListeners();
    }
  ...
  function renderCurrentTab() {
    ...
    switch (window._currentTab) {
      case 'dashboard':  window._renderDashboardTab();  break;
      case 'products':   window._renderProductsTab();   break;
      case 'messages':   window._renderMessagesTab();   break;
      case 'reviews':    window._renderReviewsTab();    break;
      case 'newsletter': window._renderNewsletterTab(); break;
      case 'orders':     window._renderOrdersTab();     break;
      case 'customers':  window._renderCustomersTab();  break;
      case 'vendors':    window._renderVendorsTab();    break;
      default: break;
    }
  }
  ...
  function loadProducts() {
    ...
    var query = isSuperAdmin()
      ? productsRef.get()
      : productsRef.where('vendorId','==', window._currentVendorId || '__none__').get();
