(function () {
  ...
  var ChatState = (function () {
    ...
    return {
      ...
      upsertSession: function (sid, data) { _sessions[sid] = Object.assign({}, _sessions[sid] || {}, data); return Object.assign({}, _sessions[sid]); },
  ...
  var ChatDB = {
    ...
    loadAndSubscribeMessages: function (sessionId, onInitial, onNewMessage, onSubReady, onError) {
      ...
      var gapCb = function (childSnap) {
        ...
        if (!initialDone) buffer.push(msg);
        else if (!initialKeys.has(msg._key)) onNewMessage(msg);
      };
      gapRef.on('child_added', gapCb, function (e) { onError && onError(e); });
      ...
      msgsRef.orderByKey().limitToLast(Cfg.MSG_INITIAL).once('value').then(function (snap) {
        ...
        buffer.forEach(function (msg) { if (!initialKeys.has(msg._key)) onNewMessage(msg); });
        buffer = [];
        gapRef.off('child_added', gapCb);
        var lastKey = msgs.length > 0 ? msgs[msgs.length - 1]._key : null;
        var liveRef = lastKey ? (U.sdkHasStartAfter ? msgsRef.orderByKey().startAfter(lastKey) : msgsRef.orderByKey().startAt(lastKey)) : msgsRef.orderByKey().limitToLast(1);
        var liveCb = function (childSnap) { onNewMessage(Object.assign({ _key: childSnap.key }, childSnap.val())); };
        liveRef.on('child_added', liveCb, function (e) { onError && onError(e); });
        onSubReady({ ref: liveRef, cb: liveCb, event: 'child_added' });
      }).catch(function (err) { gapRef.off('child_added', gapCb); onError && onError(err); });
    },
    loadOlderMessages: function (sessionId, beforeKey, onMessages, onError) {
      rtdb.ref(CHAT_ROOT + '/' + sessionId + '/messages').orderByKey().endBefore(beforeKey).limitToLast(Cfg.MSG_PAGE).once('value').then(function (snap) {
        var msgs = [];
    ...
    markSessionAsRead: function (sessionId, gen) {
      rtdb.ref(CHAT_ROOT + '/' + sessionId + '/messages').orderByChild('read').equalTo(false).once('value').then(function (snap) {
        if (ChatState.getReadGen() !== gen) return;
        var updates = {};
        snap.forEach(function (child) {
          var msg = child.val();
          if (msg && msg.sender === 'customer') updates[CHAT_ROOT + '/' + sessionId + '/messages/' + child.key + '/read'] = true;
        });
    ...
    setAdminTyping: function (sessionId, val) {
      return rtdb.ref(CHAT_ROOT + '/' + sessionId + '/meta/adminTyping/' + getAdminId()).set(val ? true : null).catch(function (e) {});
    },
    ...
    loadNote: function (sessionId) { return rtdb.ref(CHAT_ROOT + '/' + sessionId + '/meta/adminNote').once('value'); },
    lockNote: function (sessionId) {
      ...
      return ref.once('value').then(function (snap) {
        ...
        return ref.set({ adminId: adminId, ts: firebase.database.ServerValue.TIMESTAMP }).then(function () { return { locked: false }; });
      });
    },
    ...
    lookupOrders: function (sessionId) { return ordersRef.where('chatSessionId', '==', sessionId).limit(10).get(); }
  };
  ...
  var ChatRenderer = {
    ...
    _buildCard: function (sid, s) {
      ...
      var badge = s.unreadCount > 0 ? '<span class="session-unread-count">' + Number(s.unreadCount) + '</span>' : (s.pinned ? '<span class="badge badge-processing" style="font-size:9px;">Pinned</span>' : '');
  ...
  var ChatController = {
    ...
    openSession: function (sessionId) {
      ...
      ChatDB.loadAndSubscribeMessages(sessionId,
        ...
        function (msg) { if (ChatState.getActiveSid() !== sessionId) return; var tempKey = (msg.sender === 'admin') ? ChatState.confirmOptimistic(msg) : null; var added = ChatState.appendMessage(msg); if (!added && !tempKey) return; ChatRenderer.appendMessage(msg, safeEl('chat-messages-panel'), tempKey); if (msg.sender === 'customer') ChatDB.markSessionAsRead(sessionId, ChatState.bumpReadGen()); },
        ...
        function () { if (ChatState.getActiveSid() !== sessionId) return; self._retryMsgSub(sessionId, 0); }
      );
      ChatState.registerSub('typing', ChatDB.subscribeTyping(sessionId, function (isTyping) { if (ChatState.getActiveSid() === sessionId) ChatRenderer.setTypingVisible(isTyping); }));
    },
    ...
    sendMessage: function (sessionId) {
      ...
      ChatDB.sendMessage(sessionId, text).catch(function (err) {
        ...
        if (tempKey) ChatState.confirmOptimistic({ _key: '__evict__' }); if (input) input.value = text;
    ...
    togglePin: function (sessionId) {
      ...
      ChatDB.togglePin(sessionId, btn ? btn.getAttribute('data-pinned') === '1' : false).then(function (newPinned) { ChatRenderer.setPinButton(newPinned); showToast(newPinned ? 'Chat pinned' : 'Chat unpinned'); }).catch(function (e) { showToast('Error: ' + e.message, 'error'); });
    },
