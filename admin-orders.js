(function () {
  ...
  window._renderOrdersTab = function () {
    ...
    mc.innerHTML +=
      ...
      '<div id="orders-table-wrap">' +
        '<div class="empty-state"><div class="empty-state-text">Loading orders...</div></div>' +
      '</div>';
  ...
  /* ─────────────────────────────────────────────────────────
     LOAD ORDERS — always fresh from Firestore
  ───────────────────────────────────────────────────────── */
  function loadOrders() {
    ...
    query.get().then(function (snap) {
      /* Always replace cache with fresh Firestore data */
  ...
  function renderOrdersTable(orders) {
    ...
    if (filtered.length === 0) {
      var isFiltered = statusFilter || paymentFilter || search;
      wrap.innerHTML =
      ...
      filtered.map(function (o) {
        return '<tr onclick="window._openOrderDetail(\'' + esc(o.id) + '\')">' +
          ...
          '<td onclick="event.stopPropagation()">' +
            '<button class="btn btn-xs btn-ghost" onclick="window._openOrderDetail(\'' + esc(o.id) + '\')">View</button>' +
          '</td>' +
        '</tr>';
      }).join('') +
      '</tbody></table></div>';
  ...
  function renderNewOrderForm(products, draftId, draftData) {
    ...
    /* Selected line items stored in JS */
    ...
    mc.innerHTML =
        ...
        '<div style="padding:12px 16px;">' +
          '<select id="no-product-picker" class="filter-select" style="width:100%;margin-bottom:10px;" onchange="window._noPickProduct(this)">' +
            ...
            products.map(function (p) {
              ...
              return '<option value="' + esc(p.id) + '" data-name="' + esc(p.name || '') + '" data-price="' + price + '" data-brand="' + esc(p.brand || '') + '">' +
      ...
      '<div class="card" style="margin-bottom:10px;">' +
        ...
        '<div class="form-row">' +
          '<div class="form-group" style="padding:0 16px;">' +
            ...
            '<select id="no-payment-status">' +
              '<option value="unpaid"' + (d.paymentStatus === 'unpaid' || !d.paymentStatus ? ' selected' : '') + '>Unpaid</option>' +
          ...
          '<div class="form-group" style="padding:0 16px;">' +
            ...
            '<select id="no-payment-method">' +
              '<option value="eft"'        + (d.paymentMethod === 'eft'        ? ' selected' : '') + '>EFT / Bank Transfer</option>' +
        ...
        '<div class="form-row" style="margin-top:0;">' +
          '<div class="form-group" style="padding:0 16px;">' +
            ...
            '<input id="no-shipping" type="number" min="0" placeholder="0.00" value="' + esc(String(d.shippingFee || '0')) + '" oninput="window._noRecalcTotal()">' +
          '</div>' +
          '<div class="form-group" style="padding:0 16px;">' +
            '<label>Discount (R)</label>' +
            '<input id="no-discount" type="number" min="0" placeholder="0.00" value="' + esc(String(d.discount || '0')) + '" oninput="window._noRecalcTotal()">' +
          '</div>' +
        '</div>' +
        /* Order total summary */
        '<div id="no-totals" style="margin:4px 16px 14px;background:var(--surface2);border:0.5px solid var(--border);border-radius:var(--r-sm);overflow:hidden;">' +
        '</div>' +
      '</div>' +
      ...
      '<div class="card" style="margin-bottom:10px;">' +
        ...
        '<div class="form-group">' +
          '<label>Notes</label>' +
          '<textarea id="no-notes" placeholder="Internal notes — not visible to customer...">' + esc(d.internalNotes || '') + '</textarea>' +
        '</div>' +
      '</div>' +
    ...
    /* Render existing items if editing draft */
    renderOrderItems();
  ...
  window._noPickProduct = function (select) {
    ...
    var existing = window._newOrderItems.find(function (i) { return i.productId === id; });
    if (existing) {
      existing.qty++;
    } else {
      window._newOrderItems.push({ productId: id, name: name, price: price, brand: brand, qty: 1 });
    }
    ...
    listEl.innerHTML = items.map(function (item, idx) {
      return '<div class="no-item-row">' +
        '<div style="flex:1;min-width:0;">' +
          ...
          '<div style="font-size:11px;color:var(--muted);margin-top:2px;">' + fmt(item.price) + ' each</div>' +
        '</div>' +
        '<div style="display:flex;align-items:center;gap:8px;flex-shrink:0;">' +
          ...
          '<span style="font-size:13px;font-weight:500;min-width:48px;text-align:right;">' + fmt(item.price * item.qty) + '</span>' +
  ...
  window._noRecalcTotal = function () {
    ...
    totalsEl.innerHTML =
      ...
      (discount > 0 ? '<div class="info-row"><span class="label">Discount</span><span style="color:var(--success);">− ' + fmt(discount) + '</span></div>' : '') +
      '<div class="info-row" style="border-top:0.5px solid var(--border);font-weight:600;">' +
        '<span class="label" style="color:var(--text);font-weight:600;">Total</span>' +
        '<span style="font-size:15px;">' + fmt(total) + '</span>' +
      '</div>';
  };
  ...
  function buildOrderPayload(status) {
    ...
    return {
      ...
      source:            'manual',
      createdAt:         window._adminDB.app.firebase_.firestore.FieldValue
                           ? window._adminDB.app.firebase_.firestore.FieldValue.serverTimestamp()
  ...
  window._submitNewOrder = function (draftId) {
    ...
    /* Use server timestamp properly */
    payload.createdAt = firebase.firestore.FieldValue.serverTimestamp();

    ordersRef.add(payload).then(function (ref) {
      /* Delete draft if this was one */
      if (draftId) {
        draftsRef.doc(draftId).delete().catch(function () {});
      }
      ...
      html +=
        '<div>' +
          ...
          '<textarea id="order-note-input"' +
            ...
            ' placeholder="Internal notes...">' + esc(o.internalNotes || '') + '</textarea>' +
