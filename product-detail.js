...
function productCard(product, compactMode, isCollectionPage) {
  if(!product) return ''; if(isCollectionPage && product.id === 'janedore-leather-pouch' && S.currentCategoryPage !== 'sunglasses') return '';
  compactMode = compactMode || false; isCollectionPage = isCollectionPage || false;
  ...
  DOM.productDetail.innerHTML=
    ...
    '<div class="product-info">'+
      ...
      '<div class="size-guide-row" onclick="alert(\'Size guide coming soon.\')">'+
        '<div class="size-guide-bars"><div class="bar-track"></div><div class="bar-ball"></div></div>'+
        '<div class="size-guide-labels"><span>Runs small</span><span>True to size</span><span>Runs large</span></div>'+
      ...
      '<div class="info-tabs-wrap">'+
        ...
        '<div class="info-tab-panel active" data-tab="description">'+
          (hasDesc?'<div class="modal-desc" style="-webkit-line-clamp:unset;display:block;">'+formatDescription(product.description)+'</div>':'<p style="font-size:12px;font-weight:300;color:#111;">No description available.</p>')+
        '</div>'+
