  ...
  window._renderVendorsTab = function() {
    ...
    var timeout = setTimeout(function() {
      if (!loaded) {
        ...
        if (el) el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">⚠</div><div class="empty-state-text">Firestore permissions error. Check your security rules.</div><button class="btn btn-sm btn-outline" style="margin-top:12px;" onclick="window._renderVendorsTab()">Retry</button></div>';
      }
    }, 10000);
    ...
    vendorsRef.get().then(function(snap) {
      ...
      var el = safeEl('vendors-list');
      if (el) el.innerHTML = '<div class="empty-state"><div class="empty-state-icon">🔒</div><div class="empty-state-text">Permission denied. Fix Firestore rules.</div><button class="btn btn-sm btn-outline" style="margin-top:12px;" onclick="window._renderVendorsTab()">Retry</button></div>';
    });
  };
